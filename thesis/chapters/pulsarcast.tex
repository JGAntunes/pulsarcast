...
finish this chapter by looking into the \acrshort{rpc} messages that power Pulsarcast in
...
Kadmelia \acrshort{dht}, used for a range of purposes from peer discovery, content
...
the Kadmelia \acrshort{dht} before being forwarded through the topic dissemination trees.
...
using only the \acrshort{dht} if they want to. Afterwards, we forward the data through the
...
this fails, though, a query to the \acrshort{dht} is in order.
...
has been created and stored in the Kadmelia \acrshort{dht} does the node proceed to create
the actual topic descriptor (with the meta topic linked as a sub-topic), which
is then also persisted in the \acrshort{dht}. When any change to the original topic
...
descriptor from the Kademlia \acrshort{dht}. After some sanity checks, such as checking if
the node is already part of the dissemination tree, we use the Kadmelia \acrshort{dht} to
...
resorting to information previously stored locally by the \acrshort{dht} in its K buckets.
...
We started by defining a common interface for all of our \acrshort{rpc} messages as showed
...
of this message. Finally, the \acrshort{rpc} schema itself is an array of the messages, so
...
subscribe and unsubscribe) and how the Kadmelia \acrshort{dht} powers our underlying
system and its dissemination trees.
...
and how these are distributed across the network.
...
\ref{rpc-message}, taking a closer look at how our \acrshort{rpc} messages, built using
