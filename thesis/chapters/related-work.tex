...
Working with a \acrshort{p2p} architecture has its own set of challenges. When we
...
was key to deploy \acrshort{p2p} applications such as Gnutella~\footnote{https://web.archive.org/web/20000620113133/http://gnutella.wego.com},
...
It is built on top of a \acrshort{p2p} network, specifically CAN DHT (already
...
When focusing specifically on \acrshort{p2p} apps, the past years have brought
...
that paved the way for new \acrshort{p2p} applications that could
...
breakthrough in the \acrshort{p2p} field as it allows browsers to receive incoming
...
of UDP, a transport that has been vital in \acrshort{p2p} applications that need an
...
of the examples most worth mentioning is the \textbf{\acrfull{ipfs}}~\footnote{https://ipfs.io}, a \acrshort{p2p} hypermedia protocol designed
...
\acrshort{dag}}~\footnote{https://github.com/ipld/specs/blob/95df205ca5fdb961ec2c2265a169989fef595db1/FOUNDATIONS.md}.
...
Over the course of this chapter we looked at what defines the pub-sub paradigm.
We started by laying the ground with our taxonomy in
...
\begin{itemize}
  ...
  \item
    Subscription management and event dissemination - how it is managed the
    state that ensures subscription requirements are met. Some examples are
    \emph{rendezvous} nodes or multidimensional spaces
\end{itemize}


Afterwards, we deep dived into a series of practical systems that have been key
to the foundation of our knowledge base in \ref{relevant-pub-sub-systems}.
Systems like Gryphon, Siena, Meghdoot or the more recent Poldercast. We
analysed and compared these using our previously defined taxonomy.

We concluded our chapter by looking into the web technologies that
would be the driver of our pub-sub system in \ref{web-technologies}. We looked
at technologies like Javascript which have been at the forefront of the web,
not only due to its recently acquired ubiquity with NodeJS, but also through
tools like NPM. We also addressed new network protocols that came up, specially
those powering \acrshort{p2p} applications, like WebRTC, or transport protocols that offer
new and exciting opportunities such as uTP and QUIC. Lastly, we briefly
analysed applications like \acrshort{ipfs} and its underlying libp2p, seeking to solve
