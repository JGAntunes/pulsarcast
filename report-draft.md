...
One of the first approaches to solve the problem of managing subscriptions in a distributed/decentralised scenario was to rely on the concept of a rendezvous node. As the name suggests, in the rendezvous approach, there's a common node (or set of nodes) where, not only events meet, but also new subscriptions are issued to. The core idea is that this node can make sure that, eventually, all the system state will be synchronised (all the events will be propagated to the expected nodes and no subscription is left unattended). This doesn't mean that other nodes can't cache state though, the idea of the rendezvous is to have a basic reassurance in subscription management and event dissemination. Ideally this would be implemented in a distributed fashion, keeping as much pressure out of the rendezvous node as possible. There are many examples of this approach, some more sophisticated then others. Systems such as Gryphon (TODO reference to Gryphon), Jedi (TODO reference to Jedi), Siena (TODO reference to Siena) and Scribe (TODO reference to Scribe). This solution is quite common for systems that choose a tree representation for their subscriptions, which in fact is the case for all of the previously referred ones, since the root of the tree acts as a rendezvous node. A more detailed description of how this is done in Gryphon, Siena and Scribe will be made further along, since they have different approaches motivated by their different options in network architecture and subscription model. 

When it comes to event dissemination in a solution relying on rendezvous node, it is pretty much set up how this will be processed, since it's usually a matter of relying on the subscription representation built when issuing new subscriptions.
